...
vidObj = VideoReader('calf_raise_crop.mp4');
...
%% HybridTrack
load('calf_raise_crop_trackingResults.mat');
for i = 1:length(Fdat.Region.Fascicle.fas_x)
...
for i = 1:length(sid)-1
    ...
    imshow(cImage);
    line('Xdata', BR.FasData.pts{i}(:,1)-3*98,'Ydata', BR.FasData.pts{i}(:,2)-44, 'Color', [.3 .3 .3],'linewidth',1, 'marker','o','linestyle','--')
    line('Xdata', PT.FasData.pts{i}(:,1)-3*98,'Ydata', PT.FasData.pts{i}(:,2)-44, 'Color', [.5 .5 .5],'linewidth',1, 'marker','o','linestyle','--');
    line('Xdata', TZ.FasData.pts{i}(:,1)-3*98,'Ydata', TZ.FasData.pts{i}(:,2)-44, 'Color', [.7 .7 .7],'linewidth',1, 'marker','o','linestyle','--');
    
    line('Xdata', BR.ApoData.pts{i}(:,1)-3*98,'Ydata', BR.ApoData.pts{i}(:,2)-44, 'Color', [.3 .3 .3],'linewidth',1, 'marker','o','linestyle','--')
    line('Xdata', PT.ApoData.pts{i}(:,1)-3*98,'Ydata', PT.ApoData.pts{i}(:,2)-44, 'Color', [.5 .5 .5],'linewidth',1, 'marker','o','linestyle','--');
    line('Xdata', TZ.ApoData.pts{i}(:,1)-3*98,'Ydata', TZ.ApoData.pts{i}(:,2)-44, 'Color', [.7 .7 .7],'linewidth',1, 'marker','o','linestyle','--');
    
    line('Xdata',manual_x-3*98,'Ydata', manual_y-44, 'Color', [.5 .5 .5],'linewidth',2, 'marker','o');
    
    
    line('Xdata', fas_x(sid(i),:),'Ydata', fas_y(sid(i),:), 'Color', color(4,:).^.5,'linewidth',2, 'marker','o');
    line('Xdata', [1 vidObj.Width*3],'Ydata', polyval(super_coef(sid(i),:), [1 vidObj.Width*3]), 'Color', color(4,:).^.5,'linewidth',1,'linestyle','--')
    line('Xdata', [1 vidObj.Width*3],'Ydata', polyval(deep_coef(sid(i),:), [1 vidObj.Width*3]), 'Color', color(4,:).^.5,'linewidth',1,'linestyle','--');
    
    
    line('Xdata', fas_x_H(sid(i),:),'Ydata', fas_y_H(sid(i),:), 'Color', color(6,:),'linewidth',2, 'marker','o');
...
%%
cd([mainfolder, 'RUB_ultrasound_study\data\ultrasound\calf_raise'])
