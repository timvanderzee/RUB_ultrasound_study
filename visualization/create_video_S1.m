...
vidObj = VideoReader('exampleVideoTA.mp4');
...
for i = 1:length(Fdat.Region.Fascicle.fas_x)
    fas_x2(i,:) = -(Fdat.Region.Fascicle.fas_x_end{i} - vidObj.Width)  + vidObj.Width;
    fas_y2(i,:) = Fdat.Region.Fascicle.fas_y_end{i};
...
for i = 1:length(sid)-1
    ...
    imshow(cImage);
    line('Xdata', BR.FasData.pts{i}(:,1)-90,'Ydata', BR.FasData.pts{i}(:,2), 'Color', [.3 .3 .3],'linewidth',1, 'marker','o','linestyle','--')
    line('Xdata', PT.FasData.pts{i}(:,1)-90,'Ydata', PT.FasData.pts{i}(:,2), 'Color', [.5 .5 .5],'linewidth',1, 'marker','o','linestyle','--');
    line('Xdata', TZ.FasData.pts{i}(:,1)-90,'Ydata', TZ.FasData.pts{i}(:,2), 'Color', [.7 .7 .7],'linewidth',1, 'marker','o','linestyle','--');
    ...
    line('Xdata',manual_x-90,'Ydata', manual_y, 'Color', [.5 .5 .5],'linewidth',2, 'marker','o');

    line('Xdata', fas_x2(sid(i),:),'Ydata', fas_y2(sid(i),:), 'Color', color(4,:).^.5,'linewidth',2, 'marker','o','linestyle','-');
